...
def check(getopt):
	payloads = []
	with open('payloads_xss.txt', 'r') as f:
		for line in f:
...
def start(url,hash_key):
	...
	if url:
		core.main(url,getopt.proxy,getopt.user_agent,check(getopt),getopt.cookie,getopt.method)
	
		crawler.crawl(url,int(getopt.depth),getopt.proxy,getopt.user_agent,check(getopt),getopt.method,getopt.cookie,hash_key)
	elif getopt.single:
