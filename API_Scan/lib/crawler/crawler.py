...
class crawler:
	...
	@classmethod
	def crawl(self,base,depth,proxy,headers,level,method,cookie,hash_key):

		urls=self.getLinks(base,proxy,headers,cookie)
		
		
		with open(hash_key+'_urls.txt', 'a') as f:
			for item in urls:
				f.write("%s\n" % item)
		...
		for url in urls:
			...
			p.join()
			if depth != 0:
				self.crawl(url,depth-1,base,proxy,level,method,cookie,hash_key)
